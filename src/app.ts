...
import { ExpenseStore, LimitStore } from './store.ts';
 ...
 */
export function createApp({ store = new ExpenseStore(), limitStore = new LimitStore() }: AppDeps = {}): {
  app: Express;
  store: ExpenseStore;
  limitStore: LimitStore;
} {
  ...
  app.use('/limits', limitsRouter(limitStore));

  app.use((_req, res) => res.status(404).json({ error: 'not found' }));

  return { app, store, limitStore };
}
